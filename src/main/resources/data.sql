insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('user', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'x', '11-111',
'xxx', 'xxx', '1', '592123124');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('arken', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Lubin', '59-300',
'dolnośląskie', 'Paderewskiego', '3/2', '123123123');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('deodeo', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Wrocław', '52-340',
'dolnośląskie', 'Jana Pawła II', '92/3', '424264231');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('marek_77', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Warszawa', '50-222',
'mazowieckie', 'Kolejowa', '7', '120425423');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('handlarz123', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Gdańsk', '22-222',
'pomorskie', 'Kwiatowa', '17/5', '123009421');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('car_dealer', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Poznań', '33-333',
'wielkopolskie', 'Pastelowa', '3', '924857312');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('KOMIS_LUBIN', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Lubin', '59-300',
'dolnośląskie', 'Lipowa', '9', '917864364');
insert into user (username, password, city, zip, state, street, house_number, phone_number) values ('Danuta95', '$2y$12$WBXIL31LyKA23QZAyVl60uNVpaH8HRcHPg4iUx4Ef7J88xzWql5ka', 'Zielona Góra', '88-888',
'lubuskie', 'Fabryczna', '4', '948234123');

insert into brand (id, name) values (1, 'Audi');
insert into brand (id, name) values (2, 'BMW');
insert into brand (id, name) values (3, 'Abarth');
insert into brand (id, name) values (4, 'Acura');
insert into brand (id, name) values (5, 'Aixam');
insert into brand (id, name) values (6, 'Alfa Romeo');
insert into brand (id, name) values (7, 'Alpine');
insert into brand (id, name) values (8, 'Aro');
insert into brand (id, name) values (9, 'Asia');
insert into brand (id, name) values (10, 'Aston Martin');
insert into brand (id, name) values (11, 'Austin');
insert into brand (id, name) values (12, 'Autobianchi');
insert into brand (id, name) values (13, 'BAC');
insert into brand (id, name) values (14, 'Baic');
insert into brand (id, name) values (15, 'Befard');
insert into brand (id, name) values (16, 'Bentley');
insert into brand (id, name) values (17, 'BMW');
insert into brand (id, name) values (18, 'Bobcat');
insert into brand (id, name) values (19, 'Brilliance');
insert into brand (id, name) values (20, 'Bugatti');
insert into brand (id, name) values (21, 'Buick');
insert into brand (id, name) values (22, 'Cadillac');
insert into brand (id, name) values (23, 'Chevrolet');
insert into brand (id, name) values (24, 'Chrysler');
insert into brand (id, name) values (25, 'Citroën');
insert into brand (id, name) values (26, 'Cupra');
insert into brand (id, name) values (27, 'Dacia');
insert into brand (id, name) values (28, 'Daewoo');
insert into brand (id, name) values (29, 'Daihatsu');
insert into brand (id, name) values (30, 'Dodge');
insert into brand (id, name) values (31, 'Ferrari');
insert into brand (id, name) values (32, 'Fiat');
insert into brand (id, name) values (33, 'Ford');
insert into brand (id, name) values (34, 'Honda');
insert into brand (id, name) values (35, 'Hammer');
insert into brand (id, name) values (36, 'Hyundai');
insert into brand (id, name) values (37, 'Infiniti');
insert into brand (id, name) values (38, 'Jaguar');
insert into brand (id, name) values (39, 'Jeep');
insert into brand (id, name) values (40, 'Kia');
insert into brand (id, name) values (41, 'Lamborghini');
insert into brand (id, name) values (42, 'Lancia');
insert into brand (id, name) values (43, 'Land Rover');
insert into brand (id, name) values (44, 'Lexus');
insert into brand (id, name) values (45, 'Maserati');
insert into brand (id, name) values (46, 'Mazda');
insert into brand (id, name) values (47, 'Mercedes-Benz');
insert into brand (id, name) values (48, 'Mini');
insert into brand (id, name) values (49, 'Mitsubishi');
insert into brand (id, name) values (50, 'Nissan');
insert into brand (id, name) values (51, 'Opel');
insert into brand (id, name) values (52, 'Peugeot');
insert into brand (id, name) values (53, 'Porsche');
insert into brand (id, name) values (54, 'Renault');
insert into brand (id, name) values (55, 'Saab');
insert into brand (id, name) values (56, 'Seat');
insert into brand (id, name) values (57, 'Smart');
insert into brand (id, name) values (58, 'SsangYong');
insert into brand (id, name) values (59, 'Subaru');
insert into brand (id, name) values (60, 'Suzuki');
insert into brand (id, name) values (61, 'Tesla');
insert into brand (id, name) values (62, 'Toyota');
insert into brand (id, name) values (63, 'Volkswagen');
insert into brand (id, name) values (64, 'Volvo');
insert into brand (id, name) values (65, 'Škoda');


insert into offer (id, name, brand_id, car_model, description, where_it_is, why_for_sale, is_for_negotiation, price, placed_at, user_id) values (1,
'Audi A4 B5', 1, 'A4 B5', 'Witam sprzedam Audi z automatem, bez elektroniki chodzi bez problemowo . Wyposażony w 2 poduszki powietrza, elektryczne szyby-przód, klimatyzacja ,radio ,wspomaganie kierownicy ,odpinany hak, central zamek z alarmem -multilok ,aluminiowe felgi, opony wielosezonowe kupione rok temu. Wszystkie wymiany olejów i klocki itd. robione na bieżąco . Auto po przeglądzie w lipcu. W aucie nie palono. Auto kupione z Berlina przyjechało na niemieckich numerach i w Polsce rejestrowane.',
'Lubin', 'Znudziło mi się', true, 2000, NOW(), 2);
insert into offer (id, name, brand_id, car_model, description, where_it_is, why_for_sale, is_for_negotiation, price, placed_at, user_id) values (2,
'Opel Astra GTC 1.9CDTI', 1, 'Astra GTC', 'Mam do zaoferowania sprowadzoną z NIEMIEC, jedyną w swoim rodzaju ASTRE H GTC w pakiecie OPC.
Ten egzemplarz jest wyposażony w bezawaryjny, bardzo dynamiczny a zaciszem ekonomicznym silnik 1.9 cdti 150 KM.',
'Warszawa', 'Znudziło mi się', true, 13000, NOW(), 4);

